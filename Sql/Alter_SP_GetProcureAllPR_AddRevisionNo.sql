-- Adds revision_no to the result set of SP_GetProcureAllPR so
...
AS
BEGIN
 --select codelog, prstatus, * from PRHeader order by create_dt desc;
 SET NOCOUNT ON;
 select codelog,
 --CONCAT([dbo].[FUNC_MSGPRStatusWithPO](codelog, prstatus, pr_recvpono), ' ',
 --CASE WHEN quo_return = 1 THEN ' <br/>[Procure send quotation]'
 --ELSE ''
 --END
 --)
 --as prstatus_txt,
