-- Increments revision_no (and stamps revision_dt) whenever the @prstatus = 1
-- branch (the actual "Submit" action) runs on a PRHeader row that was
-- already submitted before (prior prstatus >= 1). The very first submit
-- (draft, prstatus 0 -> 1) leaves revision_no at 0, matching the confirmed
-- test: first Submit on S03434-2607152353 -> revision_no = 0, revision_dt = NULL.
-- The @prstatus = 0 branch (Save Draft) is untouched — repeatedly saving a
-- draft before the first real submit must never count as a revision.
...
-- V302 Track revision_no / revision_dt on resubmit
-- =============================================
...
BEGIN
 ...
 IF @prstatus = 1
  BEGIN
   ...
   -- V302: capture the row's state BEFORE this update so we can tell a
   -- first-time submit (prior prstatus = 0) apart from a resubmit
   -- (prior prstatus >= 1), and carry forward its current revision_no.
   DECLARE @PRE_PRSTATUS INT;
   DECLARE @PRE_REVISION_NO INT;
   SELECT @UID_PPRID = id, @CREATEDATE = create_dt, @PRE_PRSTATUS = prstatus, @PRE_REVISION_NO = revision_no FROM PRHeader WHERE prno = @prno;
   ...
   */
   ...
   UPDATE PRHeader SET prno=@doc_number, projectno=@projectno,empcode=@empcode,approx_type=@approx_type,approx_dt=@approx_dt,invcreditno=@invcreditno,
    ...
    pub_remark = @pub_remark, update_dt = getdate(), prcurrency = @prcurrency,
    revision_no = CASE WHEN ISNULL(@PRE_PRSTATUS, 0) >= 1 THEN ISNULL(@PRE_REVISION_NO, 0) + 1 ELSE ISNULL(@PRE_REVISION_NO, 0) END,
    revision_dt = CASE WHEN ISNULL(@PRE_PRSTATUS, 0) >= 1 THEN GETDATE() ELSE revision_dt END
    WHERE id = @UID_PPRID;
